...
Standalone module containing only the `OuterRimEnv` class from the
Star Wars Galaxy Explorer Phase 2 environment (dynamic entities).

Import this file from algorithm scripts instead of main.py or train.py,
which both execute top-level demo / training code on import.

Key differences from Phase 1 (toy_sim_sw_2):
  - Requires constructor args: OuterRimEnv(n_sep, n_rep)
  - Vision radius: 7  →  15x15 local grid
  - Observation space: Dict({
        "vision"        : Box(0, 4, (15,15), uint8)      terrain + republic ships
        "danger"        : Box(0.0, 1.0, (15,15), float32) separatist threat heatmap
        "reward_memory" : Box(0.0, inf, (15,15), float32) planet reward halo map
    })
  - Mobile entities: SeperatistShip (hostile) + RepublicShip (neutral)
...
class OuterRimEnv(Env):
    def __init__(self, n_sep, n_rep):
        ...
        # Observation: local grid (15x15) with integer values, plus mission time
