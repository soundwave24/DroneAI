"""
train_sac.py  (toy_sim_sw_3_eny — dynamic entity environment)
==============================================================

SAC-Discrete training for the OuterRimEnv (Phase 2) with mobile entities.
...
Key differences from Phase 1 (toy_sim_sw_2):
    - Env constructor: OuterRimEnv(n_sep, n_rep)  (was OuterRimEnv())
    - obs_to_vec: handles "danger" and "reward_memory" instead of "seen_memory"
      * vision / 4.0  (normalise 0..4 → 0..1)
      * danger        (already 0..1)
      * reward_memory tanh-normalised (unbounded raw values → 0..1 range)
    - Model saved to Training_Star_Wars_Galaxy_Phase_2/
...
class SACConfig:
    ...
    seed: int = 0
    n_sep: int = 2                    # Separatist ships in env
    n_rep: int = 2                    # Republic ships in env
    # reward_memory normalisation scale — tanh(x / scale) maps large values to [0,1]
    reward_memory_scale: float = 10.0
...
# ---------------------------------------------------------------------------
# Observation handling (Phase 2)
# ---------------------------------------------------------------------------
def obs_to_vec(obs: TDict[str, np.ndarray], reward_memory_scale: float = 10.0) -> np.ndarray:
    """Flatten the Dict observation into a single float vector in [0, 1] (approx).

    Phase 2 keys:
        "vision"        uint8  (15, 15)  values 0-4
        "danger"        float32 (15, 15) values 0..1
        "reward_memory" float32 (15, 15) values 0..inf  (tanh-normalised)
    """
    vision = obs["vision"].astype(np.float32).reshape(-1) / 4.0
    danger = obs["danger"].astype(np.float32).reshape(-1)
    # reward_memory is unbounded; tanh maps it to (0, 1) smoothly
    reward_mem = np.tanh(obs["reward_memory"].astype(np.float32).reshape(-1)
                         / reward_memory_scale)
    return np.concatenate([vision, danger, reward_mem], axis=0)


def obs_dim_for(env: OuterRimEnv, reward_memory_scale: float = 10.0) -> int:
    sample = env.observation_space.sample()
    return obs_to_vec(sample, reward_memory_scale).shape[0]
...
def train(cfg: SACConfig = CFG):
    ...
    save_dir = os.path.join("Training_Star_Wars_Galaxy_Phase_2", "Saved RL Models")
    log_dir = os.path.join("Training_Star_Wars_Galaxy_Phase_2", "logs", "SAC")
    os.makedirs(save_dir, exist_ok=True)
    os.makedirs(log_dir, exist_ok=True)
    final_model_path = os.path.join(save_dir, "SAC_Discrete_Model_Star_Wars_Galaxy")

    env = OuterRimEnv(cfg.n_sep, cfg.n_rep)
    n_actions = env.action_space.n
    obs_dim = obs_dim_for(env, cfg.reward_memory_scale)
    ...
    obs_vec = obs_to_vec(obs_dict, cfg.reward_memory_scale)

    ep_return = 0.0
    ep_len = 0
    ep_returns = deque(maxlen=20)
    ep_planets = deque(maxlen=20)
    ep_damages = deque(maxlen=20)
    ...
    for step in range(1, cfg.total_timesteps + 1):
        ...
        next_obs_vec = obs_to_vec(next_obs_dict, cfg.reward_memory_scale)
        ...
        if done or truncated:
            ...
            ep_planets.append(info.get("visited_planets", 0))
            ep_damages.append(info.get("damages", 0))
        ...
        if step % cfg.log_interval_steps == 0:
            ...
            mean_planets = np.mean(ep_planets) if ep_planets else float("nan")
            mean_damages = np.mean(ep_damages) if ep_damages else float("nan")
            mean_cov = np.mean(ep_coverage) if ep_coverage else float("nan")
            print(
                f"step={step:7d} | sps={sps:5.0f} | "
                f"ret(mean,20)={mean_ret:7.2f} | "
                f"planets(mean,20)={mean_planets:5.2f} | "
                f"damages(mean,20)={mean_damages:5.2f} | "
