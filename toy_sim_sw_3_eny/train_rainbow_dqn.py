"""
Rainbow DQN training script for OuterRimEnv Phase 2 (dynamic entities).
...
Key differences from Phase 1 (toy_sim_sw_2):
    ...
    - obs_shape: (3, 15, 15) — 3 channels × 15×15 grid
      (vs (2, 5, 5) in Phase 1)
    - pack_obs: stacks vision/4, danger, tanh(reward_memory/10)
    - v_min=-60, v_max=200 to cover Phase 2 return range
      (damage: -30, planet: +50, 1000-step episodes with discount)
...
# reward_memory normalisation scale for pack_obs
...
class RainbowNet(nn.Module):
    def __init__(self, obs_shape: Tuple[int, int, int], n_actions: int,
                 n_atoms: int, v_min: float, v_max: float, hidden: int = 256):
        super().__init__()
        self.n_actions = n_actions
        self.n_atoms = n_atoms
        c, h, w = obs_shape  # (3, 15, 15)

        # Conv backbone — input is 15x15 with 3 channels
...
# ===================================================================
# Observation packing — Dict obs -> (3, H, W) float tensor
# ===================================================================
def pack_obs(obs: Dict[str, np.ndarray],
             reward_memory_scale: float = REWARD_MEMORY_SCALE) -> np.ndarray:
    """Pack Phase 2 Dict observation into a (3, 15, 15) float32 array.

    Channels:
        0: vision        / 4.0          → 0..1
        1: danger                       → 0..1 (already)
        2: tanh(reward_memory / scale)  → 0..1 (approx; tanh maps [0,inf)->0..1)
    """
    vision = obs["vision"].astype(np.float32) / 4.0
    danger = obs["danger"].astype(np.float32)
    reward_mem = np.tanh(obs["reward_memory"].astype(np.float32) / reward_memory_scale)
    return np.stack([vision, danger, reward_mem], axis=0)  # (3, 15, 15)
...
class RainbowAgent:
    def __init__(self, obs_shape, n_actions, device,
                 ...
                 v_min=-60.0, v_max=200.0, n_atoms=51,
...
def train(total_timesteps: int, save_path: str, log_every: int = 1000,
    ...
    obs_shape = (3, 15, 15)  # (vision + danger + reward_memory channels, H, W)
    ...
    ep_planet_counts = deque(maxlen=20)
    ep_damage_counts = deque(maxlen=20)
    ...
    for t in range(1, total_timesteps + 1):
        ...
        if done or truncated:
            ...
            ep_planets = info.get("visited_planets", 0)
            ep_damages = info.get("damages", 0)
            ep_returns.append(ep_return)
            ep_planet_counts.append(ep_planets)
            ep_damage_counts.append(ep_damages)
            obs_dict, _ = env.reset()
            obs = pack_obs(obs_dict)
            ep_return = 0.0
            ep_len = 0
        ...
        if t % log_every == 0:
            ...
            mean_planets = np.mean(ep_planet_counts) if ep_planet_counts else float("nan")
            mean_damages = np.mean(ep_damage_counts) if ep_damage_counts else float("nan")
            mean_loss = np.mean(losses) if losses else float("nan")
            print(f"[step {t:>7d}] "
                  f"ret(20)={mean_ret:8.2f}  "
                  f"planets(20)={mean_planets:5.2f}  "
                  f"damages(20)={mean_damages:5.2f}  "
